...
namespace csmc\modules\ /*PUT THE NAME OF THE INSTANCE YOU ARE USING HERE*/;
...
class login{
					...
					if(!$db->errorFlag){
						...
						if($db->countExecute("SELECT COUNT(id) FROM users WHERE email='".$Email."'") == 0){
							...
							if($Passwd == $ConfirmPasswd){
								...
								if($db->boolExecute("INSERT INTO users (email,password) VALUES ('".$Email."','".md5($Passwd.$salt)."')")){
									redirects::home("", "You have registered successfully.");
								} else {
	...
	public static function register(){
		...
		<section id="login_box">
			...
			<form id="register">
				...
				<input type="submit" value="Register" id="'.__INSTANCE__.'_login_vregister#Email&Passwd&ConfirmPasswd" class="form_button"/>
			</form>
		</section>
		';
		ios::out($wrap);
	...
	public static function profile(){
		...
		if(!$db->errorFlag){
			...
			<section id="profile_box">
				...
				<p><strong>Mobile Device:</strong> '.$deviceType.'</p>
				<br>
				...
				<form id="changepassword">
					...
					<input type="submit" id="'.__INSTANCE__.'_login_changepassword#OldPasswd&amp;NewPasswd&amp;ConfirmNewPasswd" class="form_button" value="Change" />
				</form>
				...
				<form id="changeemail">
					...
					<input type="submit" value="Change" id="'.__INSTANCE__.'_login_changeemail#NewEmail&amp;ConfirmNewEmail" class="form_button"/>
				</form>
				<br>
				...
				<form id="deleteaccount">
					<input type="submit" value="Delete" id="'.__INSTANCE__.'_login_deleteaccount" class="form_button"/>
				</form>
			</section>
			';
	...
	public static function changepassword(){
		...
		if(isset($_SESSION["post_params"]["OldPasswd"]) && isset($_SESSION["post_params"]["NewPasswd"]) && isset($_SESSION["post_params"]["ConfirmNewPasswd"])){
			...
			if(empty(trim($_SESSION["post_params"]["OldPasswd"])) || empty(trim($_SESSION["post_params"]["NewPasswd"]))||empty(trim($_SESSION["post_params"]["ConfirmNewPasswd"]))){
			...
			if(!$db->errorFlag){
				...
				$count = $db->countExecute("SELECT COUNT(id) FROM users WHERE email='".$_SESSION["csmc_modules_login_email"]."' AND password='".md5($_SESSION["post_params"]["OldPasswd"].$salt)."'");
				if($count == 1){
					...
					if($_SESSION["post_params"]["NewPasswd"] == $_SESSION["post_params"]["ConfirmNewPasswd"]){
						//Update the password
						if($db->boolExecute("UPDATE users SET password='".md5($_SESSION["post_params"]["NewPasswd"].$salt)."' WHERE `email`='".$_SESSION["csmc_modules_login_email"]."'")){
	...
	public static function login(){
		...
		<section id="login_box">
			...
			<form id="login">
				<input id="Email" name="Email" maxlength="100" type="text" placeholder="Email" autocomplete="on" spellcheck="off" required/>
				<input id="Passwd" name="Passwd" type="password" maxlength="50" placeholder="Password" required/>
				<input type="submit" value="Login" id="'.__INSTANCE__.'_login_validate#Email&amp;Passwd" class="form_button"/>
			</form>
		</section>';
		return ios::out($wrap);
	}
	...
	public static function validate(){
		...
		if($db->errorFlag === false){
			...
			if(isset($_SESSION["post_params"]["Email"]) && isset($_SESSION["post_params"]["Passwd"])){
				...
				$password = md5($_SESSION["post_params"]["Passwd"].$salt);
				...
				if($count == 1){
					...
					$last_time = $db->dataExecute("SELECT last_failed_login_try FROM users WHERE email='".$email."'", "last_failed_login_try")["last_failed_login_try"];
	...
	public static function logout(){
		...
		//Calls the native destroy method to destroy session data and redirect user
		session::destroy();
	}
	/**
	...
	public static function requireLogin(){
		if(!login::isLoggedIn()){
			...
			$time = 2000;
			redirects::home($message,$notification,$time);
			return true;
		}
		return false;
	}
